...
function removeFirstOccurrences(str, value) {
  ...
  return pos >= 0 ? str.slice(0, pos) + str.slice(pos + value.length) : str;
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < p.length; i += 1) {
    counter = vowels.includes(p[i]) ? (counter += 1) : counter;
  }
  return counter;
}
...
function isPalindrome(str) {
  ...
  const revers = str
    ...
    .replace(regExp, () => '');
  return (
    str.toLowerCase().replace(regExp, () => '') === revers.toLocaleLowerCase()
  );
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 1; i < arr.length; i += 1) {
    word = arr[i].length > word.length ? arr[i] : word;
  }
  return word;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    ...
    for (let j = 0; j < p.length; j += 1) {
      p[j] =
        p[j].toLowerCase() === p[j] ? p[j].toUpperCase() : p[j].toLowerCase();
    }
    arr[i] = p.join('');
  }
  return arr.join(' ');
